...
func (term *Range) Scan(value interface{}) error {
	...
	if start == "infinity" || start == "" {
		...
		var startDate Date
		if startDate, err = ParseDate(start, time.Local); err != nil {
			return err
		}
	...
	var endDate Date
	if endDate, err = ParseDate(end, time.Local); err != nil {
		return err
	}
...
func EntireMonth(year int, month time.Month, loc *time.Location) (term Range) {
	first := time.Date(year, month, 1, 0, 0, 0, 0, loc)
	term.End = NewDateFromTime(first.AddDate(0, 1, -1))
	term.Start = NewDateFromTime(first)
	return
}

// EntireYear creates a Range that includes the entire year
func EntireYear(year int, loc *time.Location) (term Range) {
	first := time.Date(year, 1, 1, 0, 0, 0, 0, loc)
	term.End = NewDateFromTime(first.AddDate(1, 0, -1))
	term.Start = NewDateFromTime(first)
	return
}
