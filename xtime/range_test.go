...
var (
	empty     = Empty()
	year2015  = EntireYear(2015, time.Local)
	nov       = EntireMonth(2015, 11, time.Local)
	dec       = EntireMonth(2015, 12, time.Local)
	novOnward = Range{Start: nov.Start}
	decOnward = Range{Start: dec.Start}
	untilDec  = Range{End: nov.End}
	jan       = EntireMonth(2016, 1, time.Local)
	feb       = EntireMonth(2016, 2, time.Local)
)
...
var daysTests = []struct {
	...
	{0, Empty().Days()},
	{1, OnlyToday().Days()},
	{365, EntireYear(2015, time.Local).Days()},
	{366, EntireYear(2016, time.Local).Days()},
	{29, EntireMonth(2016, 2, time.Local).Days()},
}
...
func TestRange_Error(t *testing.T) {
	...
	end := Range{End: NewDate(2015, 3, 1, time.Local)}
	if end.Error() != nil {
		t.Error("Unbounded start dates should not error")
	}

	start := Range{Start: NewDate(2015, 3, 1, time.Local)}
	if start.Error() != nil {
		t.Error("Unbounded end dates should not error")
	}

	var invalid Range
	invalid.Start = NewDate(2015, 3, 2, time.Local)
	invalid.End = NewDate(2015, 3, 1, time.Local)
...
func TestRange_Overlaps(t *testing.T) {
	...
	if !nov.Overlaps(SingleDay(NewDate(2015, 11, 30, time.Local))) {
...
var stringTests = []struct {
	...
	{"2016-02-01 to 2016-02-29", EntireMonth(2016, 2, time.Local).String()},
	{"until 2016-02-29", Range{End: NewDate(2016, 2, 29, time.Local)}.String()},
	{"2016-02-01 onward", Range{Start: NewDate(2016, 2, 1, time.Local)}.String()},
}
...
func TestRange_Union(t *testing.T) {
	...
	if NewDate(2015, 1, 1, time.Local) != union.Start {
		t.Error(
			"The union of 2015 and January 2016 should start on 2015-01-01",
		)
	}
	if NewDate(2016, 1, 31, time.Local) != union.End {
		t.Error(
...
func TestRange_Unmarshal(t *testing.T) {
	...
	open := EntireMonth(2015, 3, timezone.China)
	...
	if NewDate(2015, 3, 1, timezone.China) != open.Start {
		t.Errorf("newDate:%v,start:%v", NewDate(2015, 3, 1, time.Local).Time, open.Start.Time)
